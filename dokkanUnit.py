...
def PoissonCDF(x,mu):
    return (poisson.cdf(x,mu)-poisson.cdf(0,mu))/(1-poisson.cdf(0,mu))
...
class Unit:
    def __init__(self,ID,nCopies,HPS,skillOrbs):
        ...
        self.avg_AA_SA = self.getAvg_AA_SA()
        ...
        self.normalDefence = -(1-(1-dodgeCancelFrac)*self.P_Dodge)*(self.kit.P_guard*(maxNormalDamage*(1-avgGuardFactor)*(1-self.dmgRed)-self.avgDefPostSuper)*guardMod+(1-self.kit.P_guard)*(maxNormalDamage*(1-self.dmgRed)-self.avgDefPostSuper)*avgTypeMod)/(maxNormalDamage*avgTypeMod)
        ...
        self.attributes = [self.leaderSkill, self.SBR,self.useability, self.healing,self.special, self.support,self.apt,self.normalDefence,self.saDefencePostSuper,self.slot1Ability]
    ...
    def getAttackDistribution(self):
        ...
        for i in range(turnMax):
            Pr_N[i] = PoissonCDF(max(11-self.constantKi[i],0),self.randomKi[i])
    ...
    def getAvgAtt(self):
        ...
        avgAtt = [0]*turnMax
        for j in range(turnMax):
            avgAtt[j] = self.Pr_N[j]*(self.normal[j]+branchAtt(i,nAA[j],M_12[j],self.p1Att[j]+self.stackedAtt[j],P_AA,nProcs,P_SA[j],P_g[j],N_0[j],A_12_0[j],self.kit.SA_12_Att[j],self.HP_P_AA)) \
                    + self.Pr_SA[j]*(self.sa[j]+branchAtt(i,nAA[j],M_12[j]+self.kit.SA_12_Att[j],self.p1Att[j]+self.stackedAtt[j]+self.kit.SA_18_Att[j],P_AA,nProcs,P_SA[j],P_g[j],N_0[j],A_12_0[j],self.kit.SA_12_Att[j],self.HP_P_AA))
            if(self.kit.rarity=='LR'): # If self. is a LR
                avgAtt[j] += self.Pr_USA[j]*(self.usa[j]+branchAtt(i,nAA[j],M_12[j]+self.kit.SA_18_Att[j],self.p1Att[j]+self.stackedAtt[j]+self.kit.SA_18_Att[j],P_AA,nProcs,P_SA[j],P_g[j],N_0[j],A_12_0[j],self.kit.SA_12_Att[j],self.HP_P_AA))
        avgAtt += counterAtt
        return avgAtt
    ...
    def getNormal(self):
        kiMultiplier = KiMultiplier(self.kit.kiMod_12,self.ki)
        return self.att*(1+leaderSkillBuff)*(1+self.p1Att+self.stackedAtt)*(1+self.linkAtt_SoT)*(1+self.p2Att)*(1+self.p3Att)*kiMultiplier
    def getSA(self):
        kiMultiplier = [self.kit.kiMod_12]*turnMax
        SAmultiplier = SAMultiplier(self.kit.SA_Mult_12,self.kit.EZA,self.kit.exclusivity,self.nCopies,self.kit.SA_12_Att_Stacks,self.kit.SA_12_Att)
        return self.att*(1+leaderSkillBuff)*(1+self.p1Att)*(1+self.linkAtt_SoT)*(1+self.p2Att)*(1+self.p3Att)*kiMultiplier*(SAmultiplier+self.kit.SA_12_Att+self.stackedAtt)
    def getUSA(self):
        kiMultiplier = KiMultiplier(self.kit.kiMod_12,np.maximum(self.ki,[18]*turnMax))
        ...
        return self.att*(1+leaderSkillBuff)*(1+self.p1Att)*(1+self.linkAtt_SoT)*(1+self.p2Att)*(1+self.p3Att)*kiMultiplier*(SAmultiplier+self.kit.SA_18_Att+self.stackedAtt)
            ...
            for i in range(17,len(kitData)):
                ...
                match kitData[i,0]:
                    ...
                    case 'AA P Super':
                        ...
                        for i in range(turnMax):
                            elements = (extended[i]).strip('][').split(',')
                            if (elements[0] != ''):
                                listOfLists[i] = np.array(elements).astype('float64')
